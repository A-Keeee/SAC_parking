...
class MultiCarParking(CarParking):
    def __init__(self, render_mode: str = None, fps: int = FPS, verbose: bool =True, 
                 use_lidar_observation: bool =USE_LIDAR, use_img_observation: bool=USE_IMG,
                 use_action_mask: bool=USE_ACTION_MASK, num_agents: int = NUM_AGENTS):
        super().__init__(render_mode, fps, verbose, use_lidar_observation, use_img_observation, use_action_mask)
        self.num_agents = num_agents
        self.vehicles = [Vehicle(n_step=NUM_STEP, step_len=STEP_LENGTH) for _ in range(self.num_agents)]
        for i in range(self.num_agents):
            self.vehicles[i].color = MULTI_CAR_COLORS[i % len(MULTI_CAR_COLORS)]
        self.lidars = [LidarSimlator(LIDAR_RANGE, LIDAR_NUM) for _ in range(self.num_agents)]
        
        self.t_per_agent = [0.0] * self.num_agents
        self.arrived = [False] * self.num_agents
        self.collided = [False] * self.num_agents
        self.outbound = [False] * self.num_agents
        self.accum_arrive_reward = [0.0] * self.num_agents

    def set_level(self, level:str=None):
        if level is None:
            self.map = ParkingMapNormal(num_agents=self.num_agents)
            return
        self.level = level
        if self.level in ['Normal', 'Complex', 'Extrem']:
            self.map = ParkingMapNormal(self.level, num_agents=self.num_agents)
        elif self.level == 'dlp':
            self.map = ParkingMapDLP()

    def reset(self, case_id: int = None, data_dir: str = None, level: str = None,) -> list:
        self.t_per_agent = [0.0] * self.num_agents
        self.arrived = [False] * self.num_agents
        self.collided = [False] * self.num_agents
        self.outbound = [False] * self.num_agents
        ...
        if level is not None:
            self.set_level(level)
        initial_states = self.map.reset(case_id, data_dir)
        for i in range(self.num_agents):
            self.vehicles[i].reset(initial_states[i])
        self.matrix = self.coord_transform_matrix()
        return self.step()[0]

    def _detect_collision(self, agent_idx):
        for obstacle in self.map.obstacles:
            if self.vehicles[agent_idx].box.intersects(obstacle.shape):
                return True
        for j in range(self.num_agents):
            if agent_idx != j:
                if self.vehicles[agent_idx].box.intersects(self.vehicles[j].box):
                    return True
        return False

    def _detect_outbound(self, agent_idx):
        x, y = self.vehicles[agent_idx].state.loc.x, self.vehicles[agent_idx].state.loc.y
        return x>self.map.xmax or x<self.map.xmin or y>self.map.ymax or y<self.map.ymin

    def _check_arrived(self, agent_idx):
    ...
    def _check_time_exceeded(self, agent_idx):
        return self.t_per_agent[agent_idx] > TOLERANT_TIME

    def _check_status(self, agent_idx):
        if self._detect_collision(agent_idx):
            return Status.COLLIDED
        if self._detect_outbound(agent_idx):
            return Status.OUTBOUND
        if self._check_arrived(agent_idx):
            return Status.ARRIVED
        if self._check_time_exceeded(agent_idx):
            return Status.OUTTIME
        return Status.CONTINUE
    ...
    def _get_safety_reward(self, agent_idx):
        ...
        if min_dist < SAFETY_DIST_THRESHOLD:
            # Exponential penalty to provide continuous gradient
            return -SAFETY_REWARD_WEIGHT * 5 * math.exp(-3 * min_dist / SAFETY_DIST_THRESHOLD)
        return 0

    def _get_reward(self, agent_idx, prev_state: State, curr_state: State):
        time_cost = - np.tanh(self.t_per_agent[agent_idx] / (10*TOLERANT_TIME))

        if REWARD_WEIGHT['rs_dist_reward'] != 0:
            radius = math.tan(VALID_STEER[-1])/WHEEL_BASE
            curr_rs_path = rsCurve.calc_optimal_path(*curr_state.get_pos(), *self.map.dests[agent_idx].get_pos(), radius , 0.1)
            prev_rs_path = rsCurve.calc_optimal_path(*prev_state.get_pos(), *self.map.dests[agent_idx].get_pos(), radius, 0.1)
            norm_rs_path = rsCurve.calc_optimal_path(*self.map.starts[agent_idx].get_pos(), *self.map.dests[agent_idx].get_pos(), radius, 0.1)
            
            curr_rs_dist = curr_rs_path.L if curr_rs_path else curr_state.loc.distance(self.map.dests[agent_idx].loc)
            prev_rs_dist = prev_rs_path.L if prev_rs_path else prev_state.loc.distance(self.map.dests[agent_idx].loc)
            rs_dist_norm_ratio = norm_rs_path.L if norm_rs_path else 10.0
            rs_dist_reward = math.exp(-curr_rs_dist/rs_dist_norm_ratio) - math.exp(-prev_rs_dist/rs_dist_norm_ratio)
        else:
            rs_dist_reward = 0
        ...
        dist_diff = curr_state.loc.distance(self.map.dests[agent_idx].loc)
        angle_diff = get_angle_diff(curr_state.heading, self.map.dests[agent_idx].heading)
        prev_dist_diff = prev_state.loc.distance(self.map.dests[agent_idx].loc)
        prev_angle_diff = get_angle_diff(prev_state.heading, self.map.dests[agent_idx].heading)
        dist_norm_ratio = max(self.map.dests[agent_idx].loc.distance(self.map.starts[agent_idx].loc),10)
        angle_norm_ratio = math.pi
        dist_reward = prev_dist_diff/dist_norm_ratio - dist_diff/dist_norm_ratio
        angle_reward = prev_angle_diff/angle_norm_ratio - angle_diff/angle_norm_ratio
        ...
        union_area = vehicle_box.intersection(dest_box).area
        box_union_reward = union_area/(2*dest_box.area - union_area)
        if box_union_reward < self.accum_arrive_reward[agent_idx]:
            box_union_reward = 0 
        else:
            prev_arrive_reward = self.accum_arrive_reward[agent_idx]
            self.accum_arrive_reward[agent_idx] = box_union_reward
            box_union_reward -= prev_arrive_reward
            
        return [time_cost, rs_dist_reward, dist_reward, angle_reward, box_union_reward]

    def get_reward(self, statuses, prev_states):
        reward_infos = []
        for i in range(self.num_agents):
            reward_list = [0,0,0,0,0]
            if statuses[i] == Status.CONTINUE:
                reward_list = self._get_reward(i, prev_states[i], self.vehicles[i].state)
    ...
    def step(self, actions:list = None):
        prev_states = [v.state for v in self.vehicles]
        statuses = [Status.CONTINUE]*self.num_agents
        
        if actions is not None:
            for i in range(self.num_agents):
                if self.arrived[i] or self.collided[i] or self.outbound[i] or self._check_time_exceeded(i):
                    continue
                for simu_step_num in range(NUM_STEP):
                    prev_info = self.vehicles[i].step(actions[i], step_time=1)
                    if self._check_arrived(i):
                        self.arrived[i] = True
                        break
                    if self._detect_collision(i):
                        if simu_step_num == 0:
                            if ENV_COLLIDE:
                                self.collided[i] = True
                            self.vehicles[i].retreat(prev_info)
                        else:
                            self.vehicles[i].retreat(prev_info)
                        simu_step_num -= 1
                        break
                simu_step_num += 1
                if simu_step_num > 1:
                    del self.vehicles[i].trajectory[-simu_step_num:-1]

        self.t += 1
        for i in range(self.num_agents):
            if not (self.arrived[i] or self.collided[i] or self.outbound[i]):
                self.t_per_agent[i] += 1
                if self._detect_outbound(i):
                    self.outbound[i] = True

            statuses[i] = self._check_status(i)

        observation = self.render(self.render_mode)
        reward_infos = self.get_reward(statuses, prev_states)
        
        # Add cooperative & safety rewards
        all_arrived = all(self.arrived)
        coop_reward = 0
        if all_arrived:
            max_time = max(self.t_per_agent)
            coop_reward = COOPERATIVE_TIME_WEIGHT * (1.0 - max_time / TOLERANT_TIME)
            
        infos = []
        for i in range(self.num_agents):
            safety_reward = self._get_safety_reward(i)
            reward_infos[i]['safety_reward'] = safety_reward
            reward_infos[i]['coop_reward'] = coop_reward
            info = OrderedDict({'reward_info':reward_infos[i], 'path_to_dest':None})
            infos.append(info)

        return observation, reward_infos, statuses, infos
    ...
    def _render(self, surface: pygame.Surface):
        ...
        for i in range(self.num_agents):
            pygame.draw.polygon(
                surface, self.vehicles[i].color, self._coord_transform(self.map.start_boxes[i]), width=1)
            pygame.draw.polygon(
                surface, self.vehicles[i].color, self._coord_transform(self.map.dest_boxes[i]), width=3)
            pygame.draw.polygon(
                surface, self.vehicles[i].color, self._coord_transform(self.vehicles[i].box))

            if RENDER_TRAJ and len(self.vehicles[i].trajectory) > 1:
                render_len = min(len(self.vehicles[i].trajectory), TRAJ_RENDER_LEN)
                for j in range(render_len):
                    vehicle_box = self.vehicles[i].trajectory[-(render_len-j)].create_box()
                    pygame.draw.polygon(
                        surface, self.vehicles[i].color[:3] + (100,), self._coord_transform(vehicle_box), width=1)

    def _get_lidar_observation(self, agent_idx):
        obs_list = [obs.shape for obs in self.map.obstacles]
        for j in range(self.num_agents):
            if j != agent_idx:
                obs_list.append(self.vehicles[j].box)
        lidar_view = self.lidars[agent_idx].get_observation(self.vehicles[agent_idx].state, obs_list)
        return lidar_view
    
    def _get_targt_repr(self, agent_idx):
        dest_pos = (self.map.dests[agent_idx].loc.x, self.map.dests[agent_idx].loc.y, self.map.dests[agent_idx].heading)
        ego_pos = (self.vehicles[agent_idx].state.loc.x, self.vehicles[agent_idx].state.loc.y, self.vehicles[agent_idx].state.heading)
        rel_distance = math.sqrt((dest_pos[0]-ego_pos[0])**2 + (dest_pos[1]-ego_pos[1])**2)
        rel_angle = math.atan2(dest_pos[1]-ego_pos[1], dest_pos[0]-ego_pos[0]) - ego_pos[2]
        rel_dest_heading = dest_pos[2] - ego_pos[2]
        tgt_repr = np.array([rel_distance, math.cos(rel_angle), math.sin(rel_angle),\
    ...
    def render(self, mode: str = "human"):
        ...
        for i in range(self.num_agents):
            ...
            if self.use_img_observation:
                # Provide a dummy image of the correct shape to satisfy network
                # Since multi-car rendering is complex, we use an empty image. 
                # (64, 64, 3) will be transposed to (3, 64, 64) in observation_rescale
                observation['img'] = np.zeros((64, 64, 3), dtype=np.float32)

            if self.use_lidar_observation:
                observation['lidar'] = self._get_lidar_observation(i)
            if self.use_action_mask:
                observation['action_mask'] = self.action_filter.get_steps(observation['lidar'])
            observation['target'] = self._get_targt_repr(i)
