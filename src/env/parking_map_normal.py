...
def generate_multi_car_bay_parking_case(map_level, num_agents=NUM_AGENTS):
    '''
    Generate a bay parking case with multiple agents.
    '''
    origin = (0., 0.)
    bay_half_len = 35. # 增大停车场，容纳3辆车及空间
    bay_PARK_WALL_DIST = BAY_PARK_WALL_DIST_DICT[map_level]
    ...
    car_spacing = WIDTH + 1.2 + random()*0.4 # 随机车位间距
    n_obst = N_OBSTACLE_DICT[map_level]
    
    # 从 12 个车位中随机选 num_agents 个不重叠的
    num_total_spots = 12
    spots = pyrand.sample(range(num_total_spots), num_agents)
    spots.sort()
    
    # 计算起始偏移，使所有车位居中在 [-bay_half_len, bay_half_len] 内
    total_width = (num_total_spots - 1) * (car_spacing + 0.5)
    lot_start_x = origin[0] - total_width / 2.0
    ...
    for i in range(num_agents):
        dest_yaw = random_gaussian_num(pi/2, pi/36, pi*5/12, pi*7/12)
        dest_x = lot_start_x + spots[i] * (car_spacing + 0.5)
    ...
    # 生成其他停车位上的障碍内容（可能是车，也可能是墙/柱子，也可能是空的）
    for i in range(num_total_spots):
        if i not in spots:
            obs_x = lot_start_x + i * (car_spacing + 0.5)
            if random() < prob_huge_obst * 0.5: # 生成柱子或墙体段
                obs_yaw = random_gaussian_num(pi/2, pi/36, pi*5/12, pi*7/12)
                obs_box = State([obs_x, origin[1]+1.0, obs_yaw, 0, 0]).create_box()
                # 检查不与目标车位重叠
                obs_valid = True
                for d_box in dest_boxes:
                    if d_box.intersects(obs_box):
                        obs_valid = False
                        break
                if obs_valid:
                    obstacles.append(obs_box)
            elif random() < prob_non_critical_car: # 生成障碍车辆
                obs_yaw = random_gaussian_num(pi/2, pi/36, pi*5/12, pi*7/12)
                ...
                obs_box = State([obs_x, obs_y, obs_yaw, 0, 0]).create_box()
                # 检查不与目标车位重叠
                obs_valid = True
                for d_box in dest_boxes:
                    if d_box.intersects(obs_box):
    ...
    if False: # 暂时关闭动态随机路障，保证所有车位绝对可达
        if random() < 0.15: # 生成一堵墙
            other_obstcales = [LinearRing(( 
            (origin[0]-bay_half_len, bay_PARK_WALL_DIST+max_obstacle_y+MIN_DIST_TO_OBST),
            (origin[0]+bay_half_len, bay_PARK_WALL_DIST+max_obstacle_y+MIN_DIST_TO_OBST), 
            (origin[0]+bay_half_len, bay_PARK_WALL_DIST+max_obstacle_y+MIN_DIST_TO_OBST+0.1), 
            ...
            for _ in range(n_obst + 2): # 稍微多生成一点随机障碍物
                obs_x = random_uniform_num(*valid_obst_x_range)
                obs_y = random_uniform_num(*valid_obst_y_range)
                obs_yaw = random()*pi*2
                obs_coords = np.array(State([obs_x, obs_y, obs_yaw, 0, 0]).create_box().coords[:-1])
    ...
    # dropout 障碍物
    for obs in other_obstcales[:]:
        if random() < DROUP_OUT_OBST:
            other_obstcales.remove(obs)
    ...
    valid_start_x_range = (origin[0]-bay_half_len/2+2, origin[0]+bay_half_len/2-2)
    valid_start_y_range = (max_obstacle_y+1.5, bay_PARK_WALL_DIST+max_obstacle_y-1.5)
    ...
    if generate_success:
        ...
        return generate_multi_car_bay_parking_case(map_level, num_agents)

def generate_multi_car_parallel_parking_case(map_level, num_agents=NUM_AGENTS):
    '''
    Generate a parallel parking case with multiple agents.
    '''
    origin = (0., 0.)
    bay_half_len = 45. # 更大的范围
    para_PARK_WALL_DIST = PARA_PARK_WALL_DIST_DICT[map_level]
    ...
    car_spacing = LENGTH + 1.5 + random()*1.0
    n_obst = N_OBSTACLE_DICT[map_level]
    ...
    left_wall = LinearRing((
        (origin[0]-bay_half_len, origin[1]),
        (origin[0]-bay_half_len, origin[1]+para_PARK_WALL_DIST),
        (origin[0]-bay_half_len-1, origin[1]+para_PARK_WALL_DIST),
        (origin[0]-bay_half_len-1, origin[1])
    ))
    right_wall = LinearRing((
        (origin[0]+bay_half_len, origin[1]),
    ...
    for i in range(num_agents):
        ...
        rb, rf, _, _  = list(State([dest_x, origin[1], dest_yaw, 0, 0]).create_box().coords)[:-1]
        min_dest_y = -min(rb[1], rf[1]) + MIN_DIST_TO_OBST
        dest_y = random_gaussian_num(min_dest_y+0.4, 0.2, min_dest_y, min_dest_y+0.8)
    ...
    for i in range(num_total_spots):
        if i not in spots:
            obs_x = lot_start_x + i * (car_spacing + 0.5)
            if random() < prob_huge_obst * 0.5:
                obs_yaw = random_gaussian_num(0, pi/36, -pi/12, pi/12)
                obs_box = State([obs_x, origin[1]+1.0, obs_yaw, 0, 0]).create_box()
                obs_valid = True
                for d_box in dest_boxes:
                    if d_box.intersects(obs_box):
                ...
                if obs_valid:
                    obstacles.append(obs_box)
            elif random() < prob_non_critical_car:
                obs_yaw = random_gaussian_num(0, pi/36, -pi/12, pi/12)
                rb, rf, _, _  = list(State([obs_x, origin[1], obs_yaw, 0, 0]).create_box().coords)[:-1]
                min_obs_y = -min(rb[1], rf[1]) + MIN_DIST_TO_OBST
                obs_y = random_gaussian_num(min_obs_y+0.4, 0.2, min_obs_y, min_obs_y+0.8)
                obs_box = State([obs_x, obs_y, obs_yaw, 0, 0]).create_box()
                obs_valid = True
                for d_box in dest_boxes:
    ...
    if False: # 暂时关闭动态随机路障，保证所有车位绝对可达
        if random() < 0.15: # 生成一堵墙
            ...
            (origin[0]-bay_half_len, para_PARK_WALL_DIST+max_obstacle_y),
            (origin[0]+bay_half_len, para_PARK_WALL_DIST+max_obstacle_y), 
            (origin[0]+bay_half_len, para_PARK_WALL_DIST+max_obstacle_y+6), 
            (origin[0]-bay_half_len, para_PARK_WALL_DIST+max_obstacle_y+6)))
            valid_obst_x_range = (origin[0]-bay_half_len+2, origin[0]+bay_half_len-2)
            valid_obst_y_range = (para_PARK_WALL_DIST+max_obstacle_y+2, para_PARK_WALL_DIST+max_obstacle_y+4)
    ...
    valid_start_x_range = (origin[0]-bay_half_len/2+2, origin[0]+bay_half_len/2-2)
    valid_start_y_range = (max_obstacle_y+1.5, para_PARK_WALL_DIST+max_obstacle_y-1.5)
    ...
    for i in range(num_agents):
        ...
        while not start_box_valid and attempts < 100:
            ...
            if start_box_valid:
                # flip the dest box so that the orientation of start matches the dest
                if cos(start_yaw)<0:
                    dest_box_center = np.mean(np.array(dest_boxes[i].coords[:-1]), axis=0)
                    dests[i][0] = 2*dest_box_center[0] - dests[i][0]
                    dests[i][1] = 2*dest_box_center[1] - dests[i][1]
                    dests[i][2] += pi
    ...
    if generate_success:
        ...
        return generate_multi_car_parallel_parking_case(map_level, num_agents)
...
class ParkingMapNormal(object):
    ...
    def reset(self, case_id: int = None, path: str = None) -> List[State]:
        if (case_id == 0 or (random() > 0.5 and case_id != 1)) and self.map_level in ["Normal", "Complex"]:
            if self.num_agents > 1:
                starts, dests, obstacles = generate_multi_car_bay_parking_case(self.map_level, self.num_agents)
            else:
                starts, dests, obstacles = generate_bay_parking_case(self.map_level)
                starts, dests = [starts], [dests]
            self.case_id = 0
        else:
            if self.num_agents > 1:
                starts, dests, obstacles = generate_multi_car_parallel_parking_case(self.map_level, self.num_agents)
            else:
                starts, dests, obstacles = generate_parallel_parking_case(self.map_level)
                starts, dests = [starts], [dests]
            self.case_id = 1
        
        self.starts = [State(s+[0,0]) for s in starts]
        self.start_boxes = [s.create_box() for s in self.starts]
        self.dests = [State(d+[0,0]) for d in dests]
        self.dest_boxes = [d.create_box() for d in self.dests]
        
        # Keep `start` and `dest` for backward compatibility with 1 agent code
        self.start = self.starts[0]
        self.dest = self.dests[0]
        self.start_box = self.start_boxes[0]
        self.dest_box = self.dest_boxes[0]
        
        self.xmin = np.floor(min([s.loc.x for s in self.starts] + [d.loc.x for d in self.dests]) - 10)
        self.xmax = np.ceil(max([s.loc.x for s in self.starts] + [d.loc.x for d in self.dests]) + 10)
        self.ymin = np.floor(min([s.loc.y for s in self.starts] + [d.loc.y for d in self.dests]) - 10)
        self.ymax = np.ceil(max([s.loc.y for s in self.starts] + [d.loc.y for d in self.dests]) + 10)
