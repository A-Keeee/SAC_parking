...
class SACAgent(AgentBase):
    ...
    def load(self, path: str = None, params_only: bool = None) -> None:
        ...
        if self.load_params and len(self.check_list) > 0:
            ...
            for name, item, save_state_dict in self.check_list:
                if save_state_dict:
                    ...
                    if isinstance(item, torch.Tensor):
                        item.data.copy_(checkpoint[name])
                    else:
            ...
            # Optimizers are already loaded properly via state_dict in check_list.
            ...
            if isinstance(loaded_obj, dict):
                print(f"Warning: Checkpoint at {path} is a dict. Loading with params_only=True instead.")
                return self.load(path, params_only=True)
