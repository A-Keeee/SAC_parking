...
## Usage
### Multi-Car Cooperative Parking (New!)
We have extended the HOPE simulation to support **3-car cooperative parking**. You can evaluate this new multi-agent environment with our specific visualization script:
```Shell
cd src
python ./evaluation/visualize_multi_car.py --eval_episode 10 --visualize True
```
*Note: If you have a trained model, pass `--ckpt_path <your_model.pt>` to run with the model. Otherwise, it will sample random actions or fallback to default behavior.*

### Run a pre-trained agent (Single Car)
```Shell
cd src
